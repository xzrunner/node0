...
bool SceneNode::HasUniqueComp() const
{
	static_assert(std::is_base_of<NodeComp, T>::value,
		"T must inherit from NodeComp");

	return (m.unique_comp_bitset & (1 << GetCompTypeID<T>())) != 0;
}
...
T& SceneNode::GetUniqueComp() const
{
	...
	int idx = QueryIndexByID<std::unique_ptr<NodeComp>>(m_unique_comp, m.unique_comp_sz, id);
...
bool SceneNode::HasSharedComp() const
{
	...
	{
		auto id = GetCompTypeID<CompAsset>();
		if ((m.shared_comp_bitset & (1 << id)) == 0) {
			return false;
		} else if (std::is_same<T, CompAsset>::value) {
			return true;
		} else {
			int idx = QueryIndexByID<std::shared_ptr<NodeComp>>(m_shared_comp, m.shared_comp_sz, id);
	...
	{
		return (m.shared_comp_bitset & (1 << GetCompTypeID<T>())) != 0;
	}
}
...
T& SceneNode::AddSharedComp(TArgs&&... args)
{
	...
	CompID id = std::is_base_of<CompAsset, T>::value ?
		GetCompTypeID<CompAsset>() : GetCompTypeID<T>();
